...
def train(epoch):
	...
	return data, dec_mean, enc_mean, KLD_loss, REC_loss, train_loss
...
def kmeans_score(mean):
	...
	mean = mean.reshape((mean.size(0)*mean.size(1), mean.size(2)))
	#print(mean.size())
	...
	sil_avg1, score1 = clustering_measure(10, mean)
	sil_avg2, score2 = clustering_measure(11, mean)
	sil_avg3, score3 = clustering_measure(12, mean)
	sil_avg4, score4 = clustering_measure(13, mean)

		
	print("For n_clusters =", 10, "The average silhouette_score is :", sil_avg1)
	print("For n_clusters =", 11, "The average silhouette_score is :", sil_avg2)
	print("For n_clusters =", 12, "The average silhouette_score is :", sil_avg3)
	print("For n_clusters =", 13, "The average silhouette_score is :", sil_avg4)
...
def auc_score(score):
	...
	#print(L[:640])
	#print(score)
	AUC = roc_auc_score(L[:1280], score)
	print("AUC", AUC)
...
plt.figure(figsize =(15, 10))
plt.plot(SIL_avg1, color = 'b', label='k=10')
plt.plot(SIL_avg2, color = 'c', label='k=11')
plt.plot(SIL_avg3, color = 'k', label='k=12')
plt.plot(SIL_avg4, color = 'g', label='k=13')
plt.xlabel('epochs')
plt.ylabel('Silhouette score')
plt.title('Silhouette Score over ' + str(n_epochs) + ' epochs (Train)', loc='center')
plt.legend(loc = 'best')
directory = os.path.abspath('./figures/yahoo-16')
if not os.path.exists(directory):
    os.makedirs(directory)
plt.savefig(directory+'/VRNN Silhouette Score transition over-' + str(n_epochs)+'-epochs .png')

plt.figure(figsize =(15, 10))
plt.plot(AUC1, color = 'b', label='k=10')
plt.plot(AUC2, color = 'c', label='k=11')
plt.plot(AUC3, color = 'k', label='k=12')
plt.plot(AUC4, color = 'g', label='k=13')
plt.axhline(y=0.9146, color = 'r')
plt.xlabel('epochs')
plt.ylabel('AUC score')
plt.title('AUC Score over ' + str(n_epochs) + ' epochs (Train)', loc='center')
plt.legend(loc = 'best')
directory = os.path.abspath('./figures/yahoo-16')
if not os.path.exists(directory):
    os.makedirs(directory)
