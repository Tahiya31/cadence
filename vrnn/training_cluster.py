...
def train(epoch):
	...
	for batch_idx, data in enumerate(train_loader):
		...
		#model = load_checkpoint('saves/beedance_1_500/vrnn_state_dict_451.pth')   ## used to load saved model, need to comment when training
...
def test(epoch):
	...
	print('====> Test set loss: KLD Loss = {:.4f}, NLL Loss = {:.4f}'.format(mean_kld_loss/len(test_loader.dataset), mean_nll_loss/len(test_loader.dataset)))
...
def pair_plot (list1, list2, list3):

	list1 = np.array(list1)
	list2 = np.array(list2)
	df = pd.DataFrame({'kld_loss': list1,
						'rec_loss': list2,
						'auc score': list3})
	#print(df)
	...
	if not os.path.exists(directory):
		os.makedirs(directory)
	sns_plot.savefig(directory+'/Pairplot of loss function and auc score.png')
...
for epoch in range(1, n_epochs + 1):
	...
	train_data, dec, enc, KLD_loss, REC_loss = train(epoch)
	...
	auc1 = auc_score(score1)
	auc2 = auc_score(score2)
	auc3 = auc_score(score3)
	auc4 = auc_score(score4)
...
plt.plot(SIL_avg2, color = 'r', label='k=3')
